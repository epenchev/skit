...
HttpServer::HttpServer() {
    for (set<CreateListenerFunc>::iterator it =
            HttpServer::GetRegistry().begin();
...
void HttpServer::OnHttpRequest(HttpSessionPtr session,
    ...
    if (error) {
        ...
        for (set<HttpServer::Listener*>::iterator it = fListeners.begin();
            ...
            if (handled) {
                 ...
                 */
                HttpSession::Listener* sessListener =
                        dynamic_cast<HttpSession::Listener*>(*it);
                if (sessListener) {
    ...
    if (!handled) {
        static Buffer responseBuff((void*) fDefaultResponse.data(),
                fDefaultResponse.size());
...
void HttpServer::OnSend(const SysError& error, size_t bytesWriten) {
    ...
    LOG(logDEBUG)
            << "Session handled by the HttpServer default handler with response 404,"
...
void HttpSession::OnReceive(const SysError& error, size_t bytesRead) {
    ...
    if (bytesRead && bytesRead <= fBufferObj.Size()) {
        fReqdata += fBufferObj.Get<const char*>();
        if (string::npos == fReqdata.find("\r\n\r\n")) {
            if (bytesRead < fBufferObj.Size()) {
                LOG(logWARNING)
                        << "Buffer not full no end marker, disconnecting";
                if (fListener)
                    fListener->OnHttpRequest(shared_from_this(), fRequestObj,
                            fError);
            } else
